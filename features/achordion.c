...
bool process_achordion(uint16_t keycode, keyrecord_t* record) {
    ...
    const bool is_tap_hold = is_mt || (QK_LAYER_TAP <= keycode && keycode <= QK_LAYER_TAP_MAX);
    // Check key position to avoid acting on combos.
    const bool is_physical_pos = (record->event.key.row < 254 && record->event.key.col < 254);
