...
void slsh_exclaim_finished(qk_tap_dance_state_t *state,
                           void *user_data);
...
qk_tap_dance_action_t tap_dance_actions[] = {
    [DANCE_1] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, comma_paren_finished,
                                             comma_paren_reset),
    [DANCE_2] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, dot_minus_finished,
                                             dot_minus_reset),
    [DANCE_3] = ACTION_TAP_DANCE_FN_ADVANCED(
        NULL, slsh_exclaim_finished, slsh_exclaim_reset),
    [DANCE_4] =
        ACTION_TAP_DANCE_FN_ADVANCED(NULL, l4_tt_finished, l4_tt_reset),
    [DANCE_5] =
        ACTION_TAP_DANCE_FN_ADVANCED(NULL, l1_tt_finished, l1_tt_reset),
...
const uint16_t PROGMEM kc_t__osm_mod_lsft[] = {KC_T, OSM(MOD_LSFT),
                                               COMBO_END};
const uint16_t PROGMEM osl_l2__osl_l3[] = {OSL(L2), OSL(L3), COMBO_END};
const uint16_t PROGMEM to_l1__td_dance_5[] = {TO(L1), TD(DANCE_5),
                                              COMBO_END};
const uint16_t PROGMEM td_dance_6__to_l4[] = {TD(DANCE_6), TO(L4),
                                              COMBO_END};
...
const uint16_t PROGMEM repeat__kc_space[] = {REPEAT, KC_SPACE,
                                             COMBO_END};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [L1] = LAYOUT(KC_Q, KC_W, KC_F, KC_P, KC_B, KC_J, KC_L, KC_U, KC_Y,
                  REPEAT, KC_A, KC_R, KC_S, KC_T, KC_G, KC_M, KC_N,
                  KC_E, KC_I, KC_O, KC_Z, KC_X, KC_C, KC_D, KC_V, KC_K,
                  KC_H, TD(DANCE_1), TD(DANCE_2), TD(DANCE_3), OSL(L3),
                  OSM(MOD_LSFT), KC_SPACE, OSL(L2)),

    [L2] =
        LAYOUT(KC_BACKSLASH, KC_GRAVE, KC_HASH, KC_COLON, KC_SEMICOLON,
               KC_EQUAL, KC_7, KC_8, KC_9, KC_PLUS, KC_ESCAPE, KC_QUOTE,
               KC_LEFT_CURLY_BRACE, KC_LEFT_PAREN, KC_LEFT_BRACKET,
               KC_ASTERISK, KC_4, KC_5, KC_6, KC_MINUS, OSM(MOD_LSFT),
               KC_DOT, KC_RIGHT_CURLY_BRACE, KC_RIGHT_PAREN,
               KC_RIGHT_BRACKET, KC_0, KC_1, KC_2, KC_3, KC_TRNS,
               TO(L1), KC_ENTER, KC_BSPACE, TD(DANCE_4)),

    [L3] = LAYOUT(
        KC_NO, KC_WWW_BACK, KC_UP, LCTL(LSFT(KC_TAB)), KC_NO, KC_NO,
        LCTL(KC_TAB), KC_NO, KC_WWW_FORWARD, LSFT(KC_TAB), KC_ESCAPE,
        KC_LEFT, KC_DOWN, KC_RIGHT, KC_LGUI, OSM(MOD_LSFT),
        OSM(MOD_LCTL), OSM(MOD_LGUI), OSM(MOD_LALT), KC_TAB, KC_UNDO,
        KC_CUT, KC_COPY, KC_PASTE, KC_NO, KC_NO, KC_BSPACE, KC_DELETE,
        LGUI(KC_DOT), TO(EMACS), TD(DANCE_5), KC_TAB, KC_ENTER, TO(L4)),

    [L4] = LAYOUT(
        KC_NO, KC_MS_WH_DOWN, KC_MS_UP, KC_NO, KC_BRIGHTNESS_DOWN,
        KC_BRIGHTNESS_UP, KC_NO, KC_NO, KC_MS_WH_UP, KC_NO, KC_ESCAPE,
        KC_MS_LEFT, KC_MS_DOWN, KC_MS_RIGHT, KC_NO, KC_LEFT,
        KC_MEDIA_PREV_TRACK, KC_MEDIA_PLAY_PAUSE, KC_MEDIA_NEXT_TRACK,
        KC_AUDIO_MUTE, KC_NO, KC_MS_WH_LEFT, KC_NO, KC_NO,
        KC_AUDIO_VOL_DOWN, KC_AUDIO_VOL_UP, KC_MS_BTN1, KC_MS_BTN2,
        KC_MS_WH_RIGHT, KC_F20, TO(L1), KC_TRNS, KC_ENTER, TO(L5)),

    [L5] = LAYOUT(KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_F10, KC_F7,
                  KC_F8, KC_F9, KC_NO, KC_NO, OSM(MOD_LALT),
                  OSM(MOD_LGUI), OSM(MOD_LCTL), KC_NO, KC_F11, KC_F4,
                  KC_F5, KC_F6, KC_DELETE, TD(DANCE_6), KC_NO, KC_NO,
                  KC_NO, KC_NO, KC_F12, KC_F1, KC_F2, KC_F3, KC_NO,
                  TO(L1), KC_NO, KC_NO, TO(L2)),

    [EMACS] = LAYOUT(
        LCTL(KC_G), KC_NO, KC_NO, KC_TRNS, KC_NO, KC_TRNS, KC_NO,
        KC_TRNS, KC_TRNS, REPEAT, KC_TAB, KC_NO, LCTL(LALT(KC_R)),
        LCTL(LALT(KC_T)), KC_TRNS, LCTL(LALT(KC_SPACE)),
        LCTL(LALT(KC_F)), LCTL(LALT(KC_B)), LCTL(LALT(LGUI(KC_U))),
        KC_ENTER, LCTL(LALT(KC_A)), KC_TRNS, LALT(KC_MINUS),
        LCTL(LALT(KC_K)), KC_TRNS, LCTL(LALT(KC_U)), LCTL(LALT(KC_D)),
        LCTL(KC_LEFT_ANGLE_BRACKET), LCTL(KC_RIGHT_ANGLE_BRACKET),
        LCTL(LALT(KC_E)), TO(L1), KC_TRNS, KC_TRNS, TO(L2))};
...
static td_tap_t l2_tt_tap_state = {.is_press_action = true,
                                   .state = TD_NONE};
...
// Initialize tap structure associated with example tap dance key
static td_tap_t l3_tt_tap_state = {.is_press_action = true,
                                   .state = TD_NONE};
...
// Initialize tap structure associated with example tap dance key
static td_tap_t l1_tt_tap_state = {.is_press_action = true,
                                   .state = TD_NONE};
...
// Initialize tap structure associated with example tap dance key
static td_tap_t l4_tt_tap_state = {.is_press_action = true,
                                   .state = TD_NONE};
...
// Initialize tap structure associated with example tap dance key
static td_tap_t comma_minus_tap_state = {.is_press_action = true,
                                         .state = TD_NONE};

void comma_minus_finished(qk_tap_dance_state_t *state,
                          void *user_data) {
...
// Initialize tap structure associated with example tap dance key
static td_tap_t dot_minus_tap_state = {.is_press_action = true,
                                       .state = TD_NONE};
...
// Initialize tap structure associated with example tap dance key
static td_tap_t dot_exclaim_tap_state = {.is_press_action = true,
                                         .state = TD_NONE};

void dot_exclaim_finished(qk_tap_dance_state_t *state,
                          void *user_data) {
...
static td_tap_t comma_paren_tap_state = {.is_press_action = true,
                                         .state = TD_NONE};

void comma_paren_finished(qk_tap_dance_state_t *state,
                          void *user_data) {
...
static td_tap_t slsh_exclaim_tap_state = {.is_press_action = true,
                                          .state = TD_NONE};
...
static td_tap_t slsh_paren_tap_state = {.is_press_action = true,
                                        .state = TD_NONE};
...
void process_repeat_key(uint16_t keycode, const keyrecord_t *record) {
  if (keycode != REPEAT) {
    ...
    last_modifier =
        oneshot_mod_state > mod_state ? oneshot_mod_state : mod_state;
    if (is_caps_word_on()) {
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  // It's important to update the mod variables *after* calling
  // process_repeat_key, or else only a single modifier from the
  // previous key is repeated (e.g. Ctrl+Shift+T then Repeat produces
  // Shift+T)
  mod_state = get_mods();
...
bool achordion_split_opposite_hands(const keyrecord_t *tap_hold_record,
                                    const keyrecord_t *other_record) {
  return key_left_hand(tap_hold_record->event.key) !=
         key_left_hand(other_record->event.key);
}
...
bool achordion_chord(uint16_t tap_hold_keycode,
  ...
  return true;
  /* return achordion_split_opposite_hands(tap_hold_record,
   * other_record); */
}
